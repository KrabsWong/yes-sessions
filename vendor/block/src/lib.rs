...
// Opaque, inhabited storage: only the runtime symbol's address is used.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
...
mod tests {
    ...
    #[test]
    fn test_captured_value_dropped_after_last_block_clone() {
        use std::cell::Cell;
        use std::rc::Rc;
        ...
        assert_eq!(invoke_int_block(&block), 42);
        let copied = block.copy();
        let cloned = copied.clone();
        assert_eq!(drops.get(), 0);
        drop(copied);
        assert_eq!(drops.get(), 0);
        assert_eq!(invoke_int_block(&cloned), 42);
        drop(cloned);
