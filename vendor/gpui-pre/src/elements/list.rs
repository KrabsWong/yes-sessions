...
struct StateInner {
    ...
    // Pixel movement whose path includes rows not measured yet.
    pending_wheel_scroll: Option<ListOffset>,
...
impl ListState {
    ...
    pub fn reset(&self, element_count: usize) {
        let old_count = {
            ...
            state.pending_wheel_scroll = None;
    ...
    pub fn splice_focusable(
        ...
        for ListOffset {
            ...
            &mut state.logical_scroll_top,
            &mut state.pending_wheel_scroll,
        ]
        .into_iter()
        .flatten()
        {
    ...
    pub fn scroll_by(&self, distance: Pixels) {
        ...
        let mut offset = state
            .pending_wheel_scroll
            .unwrap_or_else(|| state.logical_scroll_top());
        offset.offset_in_item += distance;
        state.queue_pixel_scroll(offset);
    }
    ...
    pub fn scroll_px_offset_for_scrollbar(&self) -> Point<Pixels> {
        ...
        if state.pending_wheel_scroll.is_none()
...
impl StateInner {
    /// Walk row boundaries using real heights only. Stop at the first unknown row,
    /// leaving the remaining pixel distance relative to a stable row boundary.
    fn walk_pixel_scroll(
        offset: &mut ListOffset,
        count: usize,
        mut height: impl FnMut(usize) -> Option<Pixels>,
    ) -> bool {
        offset.item_ix = offset.item_ix.min(count);
        while offset.offset_in_item < px(0.) && offset.item_ix > 0 {
            let Some(previous_height) = height(offset.item_ix - 1) else {
                return false;
            };
            offset.item_ix -= 1;
            offset.offset_in_item += previous_height;
        }
        offset.offset_in_item = offset.offset_in_item.max(px(0.));
        while offset.item_ix < count {
            let Some(item_height) = height(offset.item_ix) else {
                return false;
            };
            if offset.offset_in_item < item_height {
                return true;
            }
            offset.offset_in_item -= item_height;
            offset.item_ix += 1;
        }
        offset.offset_in_item = px(0.);
        true
    }

    fn queue_pixel_scroll(&mut self, mut offset: ListOffset) {
        let original = offset;
        let resolved = Self::walk_pixel_scroll(&mut offset, self.items.summary().count, |ix| {
            self.items
        ...
        if resolved {
            self.rebase_pending_scroll(offset);
            self.pending_wheel_scroll = None;
            self.logical_scroll_top = Some(offset);
        } else {
            self.pending_scroll = None;
            self.pending_wheel_scroll = Some(original);
        }
    }

    fn resolve_pixel_scroll(
        &mut self,
        ...
        let Some(mut offset) = self.pending_wheel_scroll.take() else {
            return;
        };
        // A remeasure may have run after this wheel event; its stashed anchor
        // must not overwrite the more recent user movement.
        self.pending_scroll = None;
        let mut measured = Vec::new();
        Self::walk_pixel_scroll(&mut offset, self.items.summary().count, |ix| {
            let (_, _, item) = self.items.find::<Count, _>((), &Count(ix), Bias::Right);
            let item = item?;
            let size = item.size().unwrap_or_else(|| {
                let size = render_item(ix, window, cx).layout_as_root(
                    size(
                        available_width
    ...
    fn scroll(
        ...
        // The event delta is coalesced relative to the last painted anchor.
        // Seeking by total height here would skip every unknown (zero-height) row.
        let mut target = *scroll_top;
        target.offset_in_item -= delta.y;
        self.queue_pixel_scroll(target);
        if self.alignment == ListAlignment::Bottom && self.pending_wheel_scroll.is_none() {
            let offset = self.logical_scroll_top();
            let mut cursor = self.items.cursor::<Count>(());
            cursor.seek(&Count(offset.item_ix), Bias::Right);
            let remaining = cursor.suffix();
            let padding = self.last_padding.unwrap_or_default();
            if !remaining.summary().has_unknown_height
                && remaining.summary().height - offset.offset_in_item + padding.top + padding.bottom
                    <= height
            {
                self.pending_scroll = None;
    ...
    fn layout_items(
        ...
        self.resolve_pixel_scroll(available_width, render_item, window, cx);
        ...
        if rendered_height - scroll_top.offset_in_item < available_height
            || (self.alignment == ListAlignment::Bottom
                && rendered_height - scroll_top.offset_in_item == available_height)
        {
        ...
        while leading_overdraw < self.overdraw {
            cursor.prev();
            if let Some(item) = cursor.item() {
                // Offscreen rows can finish asynchronous content preparation
                // while they are in overdraw. Refresh their actual heights before
                // the next upward scroll uses them to cross a row boundary.
    ...
    fn set_offset_from_scrollbar(&mut self, point: Point<Pixels>) {
        ...
        if self.scrollbar_drag_start_height.is_none() {
            // ScrollableMask also uses the scrollbar handle for wheel input.
            // Outside a thumb drag this is relative movement, not an absolute
            // seek through the incomplete height tree.
            let mut anchor = self.pending_wheel_scroll.unwrap_or_else(|| {
                if self.logical_scroll_top.is_none() && self.alignment == ListAlignment::Bottom {
                    ListOffset {
                        item_ix: self.items.summary().count,
                        offset_in_item: -height + padding.top + padding.bottom,
                    }
                } else {
                    self.logical_scroll_top()
                }
            });
            let distance = -point.y - self.scroll_top(&anchor);
            if distance < px(0.) {
                self.follow_state.stop_following();
            }
            anchor.offset_in_item += distance;
            self.queue_pixel_scroll(anchor);
            return;
        }
        self.pending_wheel_scroll = None;
